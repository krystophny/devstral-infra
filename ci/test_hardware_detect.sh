...
test_config_256gb_mac() {
  ...
  local config
  config="$(best_config 192000 256000)"
...
test_config_128gb_mac() {
  ...
  local config
  config="$(best_config 96000 128000)"
  local model_size ctx
  model_size="$(echo "${config}" | cut -d'|' -f1)"
  ctx="$(echo "${config}" | cut -d'|' -f3)"
...
test_config_64gb_mac() {
  ...
  local config
  config="$(best_config 48000 64000)"
  local model_size ctx
  model_size="$(echo "${config}" | cut -d'|' -f1)"
  ctx="$(echo "${config}" | cut -d'|' -f3)"
...
test_config_24gb_vram() {
  echo "TEST: 24GB VRAM -> 24B 57K context"
  local config
  config="$(best_config 24000 64000)"
  local model_size ctx
  model_size="$(echo "${config}" | cut -d'|' -f1)"
  ctx="$(echo "${config}" | cut -d'|' -f3)"

  if [[ "${model_size}" == "24B" && "${ctx}" == "57344" ]]; then
    echo "PASS: 24GB VRAM selects 24B with 57K context"
  else
    echo "FAIL: got ${model_size} with ${ctx} context"
    return 1
  fi
...
test_config_16gb_vram() {
  echo "TEST: 16GB VRAM -> 24B 8K context"
  local config
  config="$(best_config 16000 32000)"
  local model_size ctx
  model_size="$(echo "${config}" | cut -d'|' -f1)"
  ctx="$(echo "${config}" | cut -d'|' -f3)"

  # On Mac platform, 16GB usable memory gives 8K context
  if [[ "${model_size}" == "24B" && "${ctx}" == "8192" ]]; then
    echo "PASS: 16GB usable selects 24B with 8K context"
  else
    echo "FAIL: got ${model_size} with ${ctx} context"
    return 1
  fi
...
test_list_viable_configs() {
  echo "TEST: list_viable_configs returns multiple options"
  local count
...
test_config_24gb_vram || FAILED=1
test_config_16gb_vram || FAILED=1
test_list_viable_configs || FAILED=1
